...
int ft_error(int nb)
{
	if (nb == 1) // args count error
		ft_printf("Incorrect arguments, it should be like this:\n"
		"./philo number_of_philosophers time_to_die time_to_eat time_to_sleep "
		"number_of_times_each_philosopher_must_eat (optional)");
	else if (nb == 2) // non numeric or 0 arguments
		ft_printf("Incorrect arguments, it should be numerics args greater than 0");
	return (1);
}
...
void data_allocation(t_data **data, t_global_data *global_d, pthread_t **threads)
{
	// malloc memory
	*data = malloc(sizeof(t_data) * global_d->philo_amount);
	*threads = malloc(sizeof(pthread_t) * global_d->philo_amount);
	global_d->forks_mutex = malloc(sizeof(pthread_mutex_t) * global_d->philo_amount);
}
		...
		pthread_t **threads)
{
	...
	while (i < global_d->philo_amount)
	{
		...
		pthread_create(&(*threads)[i], NULL, philo_process, (void *)&((*data)[i]));
